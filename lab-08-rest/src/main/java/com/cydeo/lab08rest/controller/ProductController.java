...
public class ProductController {
    ...
    @GetMapping       //ResponseWrapper is used to customized JSON format
    ...
    @PutMapping                          //we bind json to productDTO using @RequestBody
    ...
    public ResponseEntity<ResponseWrapper> getProductListByPriceAndQuantity(@PathVariable("price") BigDecimal price
        ...
        List<ProductDTO> productDTOS = productService.retrieveProductByPriceAndQuantity(price, quantity);
    ...
    public ResponseEntity<ResponseWrapper>retrieveByCategory(@PathVariable("id")Long categoryId){
        return ResponseEntity.ok(new ResponseWrapper("Product are successfully retrieved"
                ,productService.getByCategory(categoryId),HttpStatus.OK));
